...
describe('with an IF statement', () => {
  ...
  test('the console is printed to', async () => {
    await subject.run()
    expect(MockOutput.print).toHaveBeenNthCalledWith(1, 'foo is bar')
    expect(MockOutput.print).toHaveBeenNthCalledWith(2, 'foo is bar and wibble is 3')
    expect(MockOutput.print).toHaveBeenNthCalledWith(3, 'either foo is bar or wibble is corge')
    expect(MockOutput.print).toHaveBeenNthCalledWith(4, 'foo is not qux')
    expect(MockOutput.print).toHaveBeenNthCalledWith(5, 'wibble is less than corge')
    expect(MockOutput.print).toHaveBeenNthCalledWith(6, 'corge is greater than wibble')
    expect(MockOutput.print).not.toHaveBeenCalledWith('corge is wibble')
    expect(MockOutput.print).toHaveBeenNthCalledWith(7, 'foo is bar and wibble is 3 and corge is 4')
  })
})
