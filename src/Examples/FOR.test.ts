...
describe('with a for loop', () => {
  ...
  describe('a standard for loop', () => {
    ...
    beforeEach(() => { subject = new DScript(MockOutput, source) })
  ...
  describe('a loop which steps negatively', () => {
    ...
    test('the loop counts from 4 to 1', async () => {
      await subject.run()
      expect(MockOutput.print).toHaveBeenNthCalledWith(1, '4')
      expect(MockOutput.print).toHaveBeenNthCalledWith(2, '3')
      expect(MockOutput.print).toHaveBeenNthCalledWith(3, '2')
      expect(MockOutput.print).toHaveBeenNthCalledWith(4, '1')
    })
  })
